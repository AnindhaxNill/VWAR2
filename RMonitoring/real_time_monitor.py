...
#         self.gui = gui  # VWARScannerGUI instance
...
#                 print(f"[ERROR] Could not watch path: {path} → {e}")
#         self.observer.start()
...
#         return any(folder.lower() in path for folder in self.excluded_folders)
...
#     #     if path in self.already_scanned:
#     #         return  # Don't scan it again
#     #     self.already_scanned.add(path)
        
#     #     now = time.time()
#     #     if now - self.recent_events.get(path, 0) < 5:
#     #         return
...
#     #     print(f" line 87 real time [DEBUG] File event: {path}")
#     #     threading.Thread(target=self.wait_and_scan_file, args=(path,), daemon=True).start()

#     def _handle_event(self, path):
#         # ✅ Normalize path: full, lowercase, consistent slashes
#         path = os.path.abspath(path).replace("\\", "/").lower()
#         now = time.time()

#         # ✅ Avoid duplicate scanning
#         if path in self.recent_events and time.time() - self.recent_events[path] < 5:
#             return

#         # Ignore overly frequent events
#         if now - self.recent_events.get(path, 0) < 5:
#             return
#         self.recent_events[path] = now
...
#         try:
#             # Wait until file is fully accessible AND stable
#             while waited < max_wait:
#                 if is_file_ready(path):
#                     stable_counter += 1
#                     if stable_counter >= 3:  # ✅ 3 consecutive stable checks (~1.5s)
#                         break
#                 else:
#                     stable_counter = 0  # Reset counter if file changes
...
#             # ✅ Only scan if monitoring is active
...
#         finally:
#             # ✅ Always allow path to be rescanned later if needed
...
#                             self.gui.notify_threat_detected(file_path, [rule])
#                 except Exception as e:
#                     print(f"[ERROR] Pending file scan failed: {e}")
